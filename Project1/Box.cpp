...
Box::Box(const Vector3& min, const Vector3& max, Material* matPtr)
{
	...
	list[4] = new Rectangle(mMin[Y], mMax[Y], mMin[Z], mMax[Z], mMax[X], matPtr, RECT_TYPE::YZ);
	list[5] = new Rectangle(mMin[Y], mMax[Y], mMin[Z], mMax[Z], mMin[X], matPtr, RECT_TYPE::YZ);
...
{
	return mListPtr->Hit(r, tMin, tMax, record, depth);
}
